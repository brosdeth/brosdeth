            ...
            <div class="col-sm-12">
                <div class="card border-0 bg-transparent">
                    <div class="card-header border-0 bg-transparent px-0 py-0 mb_20">
                        <div class="mb_20">
                            <a class="page-title text-nowrap"><h4 class="text-koulen">{{$t('app.categories')}}</h4></a>
                        </div>
                        <div class="d-flex justify-content-between">
                            <header-link></header-link>
                            <div>
                                <button class="btn bg-white shadow-sm" @click="fExportExcel()"><i class="fas fa-file-excel"></i> {{ $t('app.export_excel') }}</button>
                                <button v-if="$can('Category Create')" type="button" class="btn btn-success shadow-sm ml-2" data-toggle="modal" @click="funLoadModal">{{ $t('app.add_new') }}</button>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table table-borderless table_fixed">
                                    <thead class="fixed_head">
                                        <tr>
                                            ...
                                            <th scope="col">{{ $t('app.category') }}</th>
                                    ...
                                    <tbody :class="filteredEnuiries==''&&isLoading==false?'no-data':''">
                                        <tr v-for="(item,index) in filteredEnuiries" :key="index">
                                            <td>{{index+1}}</td>
                                            <td>[  {{ item.id }} ]{{item.cat_name}}</td>
                                            <td>{{item.desc}}</td>
                                            <td>
                                                <button v-if="$can('Category Edit')" title="Edit" @click="funLoadModal(item)"  class="btn btn-dark btn-sm s-btn-xs">
                                                        <i class="fa-solid fa-pen-to-square fa-sm"></i>
                                                </button>
                                                <button v-if="$can('Category Delete')" type="button" :title="$t('app.delete')" @click="funDelete(item.id)" class="btn btn-danger btn-sm s-btn-xs">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title"><span v-if="isEdit==false">{{$t('app.add_new')}}</span><span v-else>{{$t('app.edit')}}</span></h4>
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                        </div>
                        <div class="modal-body">
                            <loading :data="form.busy"></loading>
                            <div class="col-sm-12 form-group">
                                <label for="">{{ $t('app.category_name') }} <sup class="text-danger">( * )</sup></label>
                                <input :placeholder="$t('app.category_name')" type="text" class="form-control" v-model="form.cat_name" :class="{'is-invalid': form.errors.has('cat_name') }"  >
                                <div class="valid text-danger" v-if="form.errors.has('cat_name')" v-html="form.errors.get('cat_name')" />
                            </div>
                            <div class="col-sm-12 form-group">
                                <label for="">{{$t('app.description')}}</label>
                                <textarea :placeholder="$t('app.description')" class="form-control" v-model="form.desc"  id="" rows="5"></textarea>
                            </div>
                        </div>
    ...
    computed:{
        filteredEnuiries(){
            return this.datas.data.filter(cat =>{
                return  cat.cat_name?.toLowerCase().match(this.searchTextBox.trim().toLowerCase())||
                        cat.desc?.toLowerCase().match(this.searchTextBox.trim().toLowerCase())
            })
        },
    },
    methods:{
